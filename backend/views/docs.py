...
from backend.schemas.docs import (
    ...
    QuestionAnswerRequest, DocsListResponse,
)
from backend.services.auth_bearer import JWTBearer
from backend.services.docs import list_all_pdfs
...
async def summarize_document(
    request: DocSummarizationRequest, openai_client: OpenAI = Depends(get_openai_client),
...
async def question_answer(
    request: QuestionAnswerRequest, openai_client: OpenAI = Depends(get_openai_client), token: str = Depends(security_scheme)) -> QuestionAnswerResponse:
    return await ...
